function [min_fuel_temp,min_fert_temp] = Whole_Plant_Fuel_Pump_Accident(Fuel_HX_Vol,Fuel_HX_Area,Fert_HX_Vol,Fert_HX_Area)
...
V_fdn = Fuel_HX_Vol(1); %m^3
% Volume of fuel salt in the upflow tubes
V_fup = Fuel_HX_Vol(2); %m^3
...
% exchangers, and piping, ORNL-3996 Table 3.1
...
V_tfhxdn = Fuel_HX_Vol(3); %m^3
%Tube metal Volume, upflow tubes
V_tfhxup = Fuel_HX_Vol(4); %m^3
...
A_sfhx_dn = Fuel_HX_Area(3); %m^2, Calculated for different tube numbers.
...
A_sfhx_up = Fuel_HX_Area(4); %m^2, Calculated for different tube numbers.
...
%Use fertHeatExchanger function to calculation volume and area from number
%of tubes
%[Fert_HX_Vol,Fert_HX_Area] = fertHeatExchanger(1641); % total number of tubes
%[Fert_HX_Vol,Fert_HX_Area] = fertHeatExchanger(750); % total number of tubes
% Volume of fuel salt in the tubes
V_fb = Fert_HX_Vol(1); %m^3
...
A_pbhx = Fert_HX_Area(1); % m^2, area of heat transfer for
...
A_sbhx = Fert_HX_Area(2); %m^2, Calculated for different tube number
...
pipe_bv = 2.053685 + 4*V_fb; %m^3, 100 ft^3 blanket volume in pipe+blanket hx ORNL-3996 Table 3.1
...
% H_rh_table (MJ/kg)
% P_rh_table1 (MPa)
run('ReheaterEnthalpy_satliquid.m');
...
% H_rhvap_table (MJ/kg)
% P_rh_table1 (MPa)
run('ReheaterEnthalpy_vaporization.m');
...
% H_rh_table1 (MJ/kg)
% P_rh_table1 (MPa)
run('ReheaterEnthalpy_satvapor.m');
...
% H_con_table (MJ/kg)
% P_con_table (MPa)
run('CondenserEnthalpy_satliquid.m');
...
% P_con_table (MPa)
run('CondenserEnthalpy_vaporization.m');
...
trip_time_f = [1200, 2000, 2500, 11000];
...
% rampstart = 1500; %time to begin ramped reactivity insertion
% rampend = 1520; %time to end ramped reactivity insertion
% t = (rampstart:0.01:rampend); 
% endramp = 5e-4; %value of reactivity inserted at end of ramp
...
% 
% downstart = 2500; %time to begin ramped reactivity withdrawal
% downend = 2520; %time to end ramped reactivity withdrawal
% td = (downstart:0.01:downend);
% dendramp = 5e-4; %value of reactivity withdrawn at end of ramp
...
% 
% reacttime = [0, rampstart:0.01:rampend, rampend , downstart:0.01:downend];
...
%Note that the Sim Time has been reduced from 5000 to 1700 for this
%function. This is because the most limiting fuel temperature occurs at
%about 1100 to 1500 seconds into the simulation with the first fuel pump
%that is cut. Upon cutting the second fuel pump, fuel temperatures in the
%heat exchangers or the core reach temperatures that go beyond the fuel
%liquidus limit, which is beyond the objective of this simulation.
%Therefore, only the first fuel pump has been simulated in total.
sim_time = 1700; %time to conduct simulation
...
outputTemps = sim('MSBR_Plant','SrcWorkspace','current');


%get a subset of the Fuel Salt, Fertile Salt, and Coolant Salt Temperatures
%after the trip of the first fuel pump (say from 1000 to 1500)
%After study, worst case is causing a fuel freeze and fertile 
%freeze to occur in this Fuel
%accident in the first heat exchanger to have a tripped pump
%Therefore, only sample and compare the fuel temps and fertile temps in
%first heat exchangers.
fuel_hx1_sample = getsampleusingtime(outputTemps.Fuel_Heat_Ex_1_Fuel_Temps,1100,1500);
min_fuel_temp = min(fuel_hx1_sample);
fert_hx1_sample = getsampleusingtime(outputTemps.Fert_Heat_Ex_1_Fert_Temps,1100,1500);
min_fert_temp = min(fert_hx1_sample);
end
