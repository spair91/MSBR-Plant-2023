function [max_fuel_core_temp,max_fuel_hx_temp] = Whole_Plant_Reactivity_Accident(Fuel_HX_Vol,Fuel_HX_Area,Fert_HX_Vol,Fert_HX_Area)
...
%after the trip of the first fuel pump (say from 750 to 880)
%After study, worst case is causing the fuel to reach the high temp point
%in the core exit and fuel heat exchangers.
%Therefore, only sample and compare the fuel temps in
%first heat exchanger and core.
fuel_core_sample = getsampleusingtime(outputTemps.core_fuel_temp,750,880);
max_fuel_core_temp = max(fuel_core_sample);
fuel_hx1_sample = getsampleusingtime(outputTemps.Fuel_Heat_Ex_1_Fuel_Temps,750,880);
max_fuel_hx_temp = max(fuel_hx1_sample);
end
